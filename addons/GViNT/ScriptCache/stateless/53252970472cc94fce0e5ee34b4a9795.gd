...
class Statement_0:
	...
	static func evaluate(runtime: GvintRuntimeStateless):
		...
		var result = target.callv("display_text", ["""Lorem ipsum""", [runtime.foo]])
...
class Statement_2:
	...
	static func evaluate(runtime: GvintRuntimeStateless):
		var target = runtime
		assert(target.has_method("display_text"))
		var result = target.callv("display_text", ["""dolor sit amet,""", [runtime.foo]])
		if result is GDScriptFunctionState:
			yield(result, "completed")
...
class Statement_3:
	...
	static func evaluate(runtime: GvintRuntimeStateless):
		var target = runtime
		assert(target.has_method("display_text"))
		var result = target.callv("display_text", ["""consectetur adipiscing elit.""", [runtime.foo]])
		if result is GDScriptFunctionState:
			yield(result, "completed")
...
static func create_context() -> GvintContext:
	...
	context.source_filename = "res://Story/foo.txt"
	context.statements = [
	Statement_0,
	Statement_1,
	Statement_2,
